...
void s21_intToStr(int x, char *str);
...
char* s21_strerror(int errnum) {
    ...
    static char unknown_error[30] = "Unknown error: ";
        char *array[] = { "Undefined error: 0", "Operation not permitted", "No such file or directory",     \
    ...
    static char unknown_error[30] = "Unknown error ";
        char *array[] = { "Success", "Operation not permitted", "No such file or directory", "No such process",     \
    ...
    if (errnum < 0 || errnum > MAX_ERROR) {
        s21_intToStr(errnum, buf);
        err = s21_strcat(unknown_error, buf);
    } else {
        err = array[errnum];
    }
